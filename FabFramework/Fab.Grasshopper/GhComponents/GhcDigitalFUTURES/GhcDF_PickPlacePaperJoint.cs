...
{
    public class GhcDF_PickPlacePaperJoint : GH_Component
    {

        /// <summary>
        /// Initializes a new instance of the MyComponent1 class.
        /// </summary>
        public GhcDF_PickPlacePaperJoint()
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddTextParameter("SourceElementName", "SEN", "Name of the FabElement to join.", GH_ParamAccess.list);
            pManager.AddTextParameter("TargetElementName", "TEN", "Name of the FabElement to join to.", GH_ParamAccess.list);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<string> iSourceElementName = new List<string>();
            DA.GetDataList("SourceElementName", iSourceElementName);

            List<string> iTargetElementName = new List<string>();
            DA.GetDataList("TargetElementName", iTargetElementName);
            ...
            Core.FabEnvironment.Action iPickAction = new Core.FabEnvironment.Action();
            DA.GetData("PickAction", ref iPickAction);

            Core.FabEnvironment.Action iPlaceAction = new Core.FabEnvironment.Action();
            ...
            List<FabElement> sourceMatchingFabElements = FabUtilitiesElement.GetMatchingFabElement(fabComponent, iSourceElementName);
            List<FabElement> targetMatchingFabElements = FabUtilitiesElement.GetMatchingFabElement(fabComponent, iTargetElementName);

            List<FabBeam> sourceMatchingFabBeams = sourceMatchingFabElements.OfType<FabBeam>().ToList();
            List<FabBeam> targetMatchingFabBeams = targetMatchingFabElements.OfType<FabBeam>().ToList();

            //sort matchingFabElements by matchingFabElements.Index
            sourceMatchingFabBeams.Sort((x1, y1) => x1.Index.CompareTo(y1.Index));
            targetMatchingFabBeams.Sort((x1, y1) => x1.Index.CompareTo(y1.Index));

            List<FabTaskFrame> fabTask_PickPlaceTaskList = new List<FabTaskFrame>();

            for (int i = 0; i < sourceMatchingFabBeams.Count; i++)
            {
                // The key exists, retrieve the value
                FabBeam sourceFabBeam = sourceMatchingFabBeams[i];

                List<Plane> placePlanesPaper = new List<Plane>();

                for (int j = 0; j < targetMatchingFabBeams.Count; j++)
                {

                    FabBeam targetFabBeam = targetMatchingFabBeams[j];

                    // Only iterate through beams with lower index number
                    if (sourceFabBeam.Index > targetFabBeam.Index && (sourceFabBeam.Name != targetFabBeam.Name))
                    { 
                        // Check for collision of geo1 and geo2
                        bool intersection = Rhino.Geometry.Intersect.Intersection.BrepBrep(
                        sourceFabBeam.GetDesignBeam().Geometry, targetFabBeam.GetDesignBeam().Geometry, 0.001, out Curve[] intersectionCurves, out Point3d[] intersectionPoints);
                            ...
                            foreach (Curve curve in joinedCurves)
                            {
                                ...
                                if (curve.IsClosed)
                                {
                                    ...
                                    if (areaProps != null)
                                    {
                                        ...
                                        Vector3d xAxisBaseLine = new Vector3d(sourceFabBeam.GetDesignBeam().EndPoint - sourceFabBeam.GetDesignBeam().StartPoint);
                                        xAxisBaseLine.Unitize();

                                        // 1. Get Brep from fabBeam.GetDesignBeam().Geometry
                                        Brep beamBrep = sourceFabBeam.GetDesignBeam().Geometry as Brep;
                                        if (beamBrep != null)
                                        {
                ...
                if (placePlanesPaper.Count > 0)
                {
                    ...
                    {
                        Plane placePlanePaper = placePlanesPaper[p];
                        ...
                        Plane pickPlanePaper = iStaticEnvPaper.AlignPln[staticEnvCounter];
                        ...
                        placePlanePaper = FabUtilities.OrientPlane(placePlanePaper, sourceFabBeam.RefPln_Situ, sourceFabBeam.RefPln_FabOut);

                        FabTaskFrame fabTask_pickPaper = new FabTaskFrame(iPickAction.Name + "_" + p.ToString("D3") + "_" + sourceFabBeam.Name);
                        FabTaskFrame fabTask_placePaper = new FabTaskFrame(iPlaceAction.Name + "_" + p.ToString("D3") + "_" + sourceFabBeam.Name);

                        FabTaskFrame.SetPickPlaceTask(fabTask_pickPaper, fabTask_placePaper,
                            pickPlanePaper, placePlanePaper,
                            sourceFabBeam, iPickAction, iPlaceAction, iEndeffector, iFabActor, iOffsetPick, iOffsetPlace);

                        fabTask_pickPaper.Index = p;
                        fabTask_placePaper.Index = p;

                        fabTask_pickPaper.StaticEnvs[iStaticEnvPaper.Name] = iStaticEnvPaper;
                        fabTask_placePaper.StaticEnvs[sourceFabBeam.EnvFab.Name] = sourceFabBeam.EnvFab;


                        //Rotate the planes by the given angle
                        foreach (Plane frame in fabTask_pickPaper.Main_Frames)
                        {
                            frame.Rotate(FabUtilities.DegreeToRadian(iRotation), frame.ZAxis, frame.Origin);
                        }
                        foreach (Plane frame in fabTask_placePaper.Main_Frames)
                        {
                            frame.Rotate(FabUtilities.DegreeToRadian(iRotation), frame.ZAxis, frame.Origin);
                        }
                        ...
                        Plane updatedPlacePlanePaper = fabTask_placePaper.Main_Frames[0];
                        //gete YZ plane from updatedPlacePlanePaper
                        Plane flipped_placePlanePaper = new Plane(updatedPlacePlanePaper.Origin, updatedPlacePlanePaper.YAxis, updatedPlacePlanePaper.ZAxis);

                        //moove origin of plane by half of paper_width
                        Plane rectanglePlane = flipped_placePlanePaper.Clone();
